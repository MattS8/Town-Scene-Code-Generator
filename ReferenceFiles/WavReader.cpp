...
int FindChunk(FILE *fp, LPMMCKINFO pChunk, LPMMCKINFO pParentChunk)
{
	...
	do
	{
		...
		{
			fseek(fp, 1 - nBytesToRead, SEEK_CUR);    // File-Pointer zur�cksetzen + 1 Byte vor
			nBytesReadSoFar++;
			if (nBytesReadSoFar + nBytesToRead > nBytesMax) return -1;
...
int ReadCueListFromRiffFile(std::string strFileName, int nCueArrayOffset, bool bDeleteCueListFromAudioFile/* = false*/)
{
	...
	MMCKINFO LtxtChunkInfo; // Cue-L�nge
  ...
  //  nFilePointerCueSubchunk = ftell(fp);
	// Neu: DataChunkInfo.cksize neu berechnen, damit auch WavPack-Dateien gelesen werden k�nnen:
	...
	// Falls die WAV-Datei eine Cue-List enth�lt, dann diese ins Array lesen:
	int nCuesInFile = 0;
	...
	do // "Pseudo"-Schleife, um im Fehlerfalle mit 'break' hinter die Schleife zu springen. ;-)
	{
		...
		// Wenn die Cue-List aus der WAV-Datei gel�scht werden soll, dann nur die Anzahl der Cues zur�ckgeben.
